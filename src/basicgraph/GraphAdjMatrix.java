...
public class GraphAdjMatrix extends Graph {
    ...
    public List<Integer> getDistance2(int v) {
        ...
        int[][] adjSquare = adjacencySquare();
        int[] twoHopNeighbours = adjSquare[v];
        List<Integer> twoHopsList = new ArrayList<>();

        for (int i = 0; i < twoHopNeighbours.length; i++) {
            if (twoHopNeighbours[i] > 0) {
                int count = twoHopNeighbours[i];
                while (count > 0) {
                    twoHopsList.add(i);
                    count--;
                }
    ...
    private int[][] adjacencySquare() {

        int[][] twoHops = new int[getNumVertices()][getNumVertices()];

        for (int i = 0; i < getNumVertices(); i++) {
            for (int j = 0; j < getNumVertices(); j++) {
                for (int k = 0; k < getNumVertices(); k++) {
                    twoHops[i][j] += adjMatrix[i][k] * adjMatrix[k][j];
                }
            }
        }
